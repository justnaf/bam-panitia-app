<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            <a href="{{route('events.index')}}">Events Management</a> > <span>Edit</span>
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mt-4">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{route('events.update',$event->id)}}" class="max-w-lg mx-auto" x-data='eventsForm' x-ref='eForm'>
                        @csrf
                        @method('PUT')
                        @if($event->status == 'submission' || $event->status == 'draft')
                        <div class="grid grid-cols-2 gap-4 mb-3">
                            <div class="w-full px-1">
                                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="name">
                                    Nama Kegiatan
                                </label>
                                <input class="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md p-2 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="BAM" type="text" name="name" id="name" value="{{$event->name}}" />
                            </div>
                            <div class="w-full px-1">
                                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="institution">
                                    Penyelenggara
                                </label>
                                <input class="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md p-2 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="LP2SI UNIMMA" type="text" name="institution" id="institution" value="{{$event->institution}}" />
                            </div>
                            <div class="w-full px-1">
                                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="location">
                                    Lokasi
                                </label>
                                <input class="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md p-2 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Auditorium UNIMMA" type="text" name="location" id="location" value="{{$event->location}}" />
                            </div>
                            <div class="w-full px-1">
                                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="location_url">
                                    Link Gmaps Lokasi
                                </label>
                                <input class="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md p-2 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="https://" type="text" name="location_url" id="location_url" value="{{$event->location_url}}" />
                            </div>
                            <div class="w-full px-1">
                                <div x-data="{ 
                                    open: false, 
                            ...
                            <div class="w-full px-1 col-span-2">
                                ...
                                <input class="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md p-2 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="9999" type="number" name="max_person" id="max_person" value="{{$event->max_person}}" />
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-3">
                            ...
                            <div class="w-full px-1">
                                ...
                                <select class="placeholder:italic placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md p-2 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" id="status" name="status">
                                    <option value="preparation" @if($event->status == 'preparation') selected @endif>Persiapan</option>
                                    <option value="registration" @if($event->status == 'registration') selected @endif>Registrasi</option>
                                    <option value="on-going" @if($event->status == 'on-going') selected @endif>Sedang Berlangsung</option>
                                    <option value="done" @if($event->status == 'done') selected @endif>Selesai</option>
                                </select>
                            </div>
                            <div class="w-full px-1">
                                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="location_url">
